...
"""
crypto_buy_score.py
- Fetches 15m OHLCV from Binance (ccxt)
- Computes indicators & a simple buy score
- Appends to docs/buy_scores.csv
...
TIMEFRAME = "15m"
LOOKBACK_HOURS = 12             # chart window
UI_INTERVAL_MINUTES = 15        # shown in UI only
...
def fetch_df(sym, timeframe, hours):
...
<script>
const LOOKBACK_HOURS=__LOOKBACK__, UI_INTERVAL_MINUTES=__UI_INTERVAL__, FRESH_MS=10*60*1000;
...
function buildTimeline(lastUtc,hours){const end=new Date(lastUtc),start=new Date(end.getTime()-hours*3600000);const out=[],step=900000,alignedEnd=floor15(end);
  for(let t=floor15(start);t<=alignedEnd;t=new Date(t.getTime()+step)) out.push(new Date(t).toISOString()); return out;}
function seriesFromRows(rows,timeline){const bySym={},syms=new Set(rows.map(r=>r.symbol));syms.forEach(s=>bySym[s]=new Array(timeline.length).fill(null));
  const idx=new Map(timeline.map((t,i)=>[t,i])); rows.sort((a,b)=>new Date(a.timestamp_utc)-new Date(b.timestamp_utc));
  rows.forEach(r=>{const b=floor15(new Date(r.timestamp_utc)).toISOString(); const i=idx.get(b); if(i===undefined)return; const v=parseInt(r.score,10); if(!Number.isNaN(v)) bySym[r.symbol][i]=v;});
  return bySym;}
function renderTable(rows){const cols=["timestamp_utc","symbol","score","close","ema20","ema50","rsi","macd","macd_signal","macd_hist","bb_low","bb_mid","bb_high"];
  ...
  rows.forEach(r=>{h+='<tr>'+cols.map(c=>'<td>'+(r[c]??'')+'</td>').join('')+'</tr>'}); document.getElementById('tableWrap').innerHTML=h+'</tbody></table>';}
async function loadAndUpdate(){try{
  ...
  const palette=["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b","#e377c2","#7f7f7f","#bcbd22","#17becf"], syms=Object.keys(series);
  chart.data.datasets=syms.map((s,i)=>({label:s,data:series[s],borderColor:palette[i%palette.length],backgroundColor:palette[i%palette.length],spanGaps:true,tension:.25,pointRadius:3,pointHoverRadius:5}));
  chart.update('none');
...
setInterval(loadAndUpdate,30000);
    ...
    html=(html.replace("__LAST_RUN__",last_run_iso)
...
def run_once():
    ...
    for sym in SYMBOLS:
        try:
            df=fetch_df(sym,TIMEFRAME,LOOKBACK_HOURS); df=add_indicators(df); last=df.iloc[-1]; score=score_row(last)
            print(f"{sym}: {score}/100 | Close={last['close']:.2f} | RSI={last['rsi']:.1f}")
            rows.append({"timestamp_utc":now,"symbol":sym,"score":int(score),
    ...
    if rows: log_to_csv(rows); write_html_from_csv(); git_push()
    else: print("No rows written; skipping push.")
